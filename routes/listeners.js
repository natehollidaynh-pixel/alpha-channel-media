...
// Get listener's own profile (requires JWT)
router.get('/me', async (req, res) => {
  try {
    ...
    const db = req.app.locals.db;
    const result = await db.query(
      'SELECT id, username, email, first_name, last_name, created_at FROM listeners WHERE id = $1',
      [decoded.id]
    );

    if (result.rows.length === 0) return res.status(404).json({ error: 'Listener not found' });
    res.json({ listener: result.rows[0] });
  } catch (err) {
    console.error('Listener profile error:', err);
    res.status(500).json({ error: 'Server error' });
  }
});

// Update listener's profile (requires JWT)
router.put('/me', async (req, res) => {
  try {
    ...
    // Check if email is taken by another listener
    const emailCheck = await db.query(
      'SELECT id FROM listeners WHERE LOWER(email) = LOWER($1) AND id != $2',
      [email, decoded.id]
    );
    ...
    const result = await db.query(
      'UPDATE listeners SET first_name = $1, last_name = $2, email = $3 WHERE id = $4 RETURNING id, username, email, first_name, last_name, created_at',
      [first_name, last_name, email, decoded.id]
    );

    res.json({ success: true, listener: result.rows[0] });
  } catch (err) {
    console.error('Update listener profile error:', err);
    res.status(500).json({ error: 'Server error' });
  }
});

// Change listener's password (requires JWT + current password)
router.put('/change-password', async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT password_hash FROM listeners WHERE id = $1', [decoded.id]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Listener not found' });
    ...
    await db.query('UPDATE listeners SET password_hash = $1 WHERE id = $2', [newHash, decoded.id]);
